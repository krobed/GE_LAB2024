...
class SymbolicRegression():
    ...
    def read_fit_cases(self):
        self.df_25 = pd.read_csv('resources/LIB/CI/df_n_25.txt',sep=',').sample(n=1000, random_state=1)
        self.X_25 = self.df_25.values[:,:-1]
        self.Y_25 = self.df_25.values[:,-1]
        self.df_53 = pd.read_csv('resources/LIB/CI/df_n_53.txt',sep=',').sample(n=1000, random_state=1)
        self.X_53 = self.df_53.values[:,:-1]
        self.Y_53 = self.df_53.values[:,-1]
        self.df_74 = pd.read_csv('resources/LIB/CI/df_n_74.txt',sep=',').sample(n=1000, random_state=1)
        self.X_74 = self.df_74.values[:,:-1]
        self.Y_74 = self.df_74.values[:,-1]
        self.df_102 = pd.read_csv('resources/LIB/CI/df_n_102.txt',sep=',').sample(n=1000, random_state=1)
    ...
    def evaluate(self, individual):
        ...
        fitness_train = np.mean([error_102,error_74,error_25])
...
p = r"Constant"
aux = 'Constant*(x[0]**Constant)*(x[1]**Constant)*x[2]'
...
opt_const = res['x']
# ind = [[0],[0],[3,2],[2],[2],[1,0],[0]]
# mapping_values = [0 for i in ind]
# # phenotype, tree_depth = grammar_sge.mapping(ind, mapping_values)
# p = r"Constant"
# replace_phenotype = 'Constant*(x[0]**Constant)*(x[1]**Constant)*x[2]'

# n_constants = len(re.findall(p, replace_phenotype))
# for i in range(n_constants):
#     replace_phenotype = replace_phenotype.replace('Constant', 'c[' + str(i) + ']',1)
print(opt_const)
